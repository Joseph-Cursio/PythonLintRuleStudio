...
class AnalyticsWindow(ctk.CTkToplevel):
    def __init__(self, master, controller):
        ...
        self.title("Quality Analytics & Trends")
        ...
        ctk.CTkLabel(
            header_frame, text="Code Quality Insights", 
            font=("", 24, "bold")
        ).pack(side="left")
    ...
    def _populate_analytics(self):
        ...
        ctk.CTkLabel(
            summary_frame, text=f"Latest Scan: {latest_count} Violations", 
            font=("", 18, "bold")
        ).pack(pady=(10, 0))
        
        ctk.CTkLabel(
            summary_frame, text=f"Trend: {delta_text}", 
        ...
        ctk.CTkLabel(
            author_frame, text="Violations by Author", 
            font=("", 14, "bold")
        ).pack(pady=10)
        ...
        if not authors:
            ...
            for author, count in authors.items():
                ...
                ctk.CTkLabel(
                    author_frame, text=f"{name}: {count}"
        ...
        ctk.CTkLabel(
            hotspot_frame, text="Top Rule Hotspots", 
            font=("", 14, "bold")
        ).pack(pady=10)
        ...
        if not rules:
            ...
            for rule_id, count in rules.items():
                ctk.CTkLabel(
                    hotspot_frame, text=f"{rule_id}: {count}"
                ).pack(anchor="w", padx=20)
        ...
        ctk.CTkLabel(
            history_frame, text="Recent Scan History", 
            font=("", 14, "bold")
        ).pack(pady=10)
        
        for run in history:
            # run: (id, timestamp, branch, total_violations)
            ts = run[1][:19] # Truncate microseconds
            branch = run[2] if run[2] else "N/A"
            ctk.CTkLabel(
                history_frame, 
                text=f"{ts} | Branch: {branch} | Count: {run[3]}"
